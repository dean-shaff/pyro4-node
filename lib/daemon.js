...
class Daemon{
    ...
    get_metadata(objectId){
        if (objectId in this.registeredObjects){
            ...
            throw new DaemonError(`get_metadata: No object registered with object ID ${objectId}`)
        }
    }
    ...
    _onSocketData(socket){
        return async (data)=>{
            ...
            for (let msg of msgs){
                Daemon.logger.debug(`_onSocketData: msg.type: ${msg.type}`)
            ...
            //     Daemon.logger.debug(
            //         `_onSocketData: received ${msgs.length} messages`)
            //     for (let msg of msgs){
            //         Daemon.logger.debug(`_onSocketData: msg.type: ${msg.type}`)
            //         if (msg.type === message.MSG_INVOKE){
            //             await this._handleInvoke(socket, msg)
...
expose(Daemon.prototype.register)
