...
struct.prototype.sizeOf = function(format){
    return pythonStruct.sizeOf(format);
}
...
Message.headerSize = Message.headerStruct.sizeOf(Message.headerFormat);
...
Message.recv = function(data){
    ...
    processData = function(data, start){
        ...
        var tar = headerData[0]; var ver = headerData[1]; var msgType = headerData[2];
        var flags = headerData[3]; var seq = headerData[4]; var dataSize = headerData[5];
        var serializerId = headerData[6]; var annotationsSize = headerData[7]; var checksum = headerData[9];
