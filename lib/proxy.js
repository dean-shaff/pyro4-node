...
class Proxy{
    ...
    batch(calls, initErrorHandler){
        ...
        this.remoteCallQueue.push({emit:this._remoteHandShake(), consume:this._remoteHandShakeConsume(), error:initErrorHandler})
        calls.forEach((callObj)=>{
            if (callObj.constructor === Function){
                this.remoteCallQueue.push(callObj())
            }else{
                this.remoteCallQueue.push(this.deferedCall(callObj.methodName, callObj.options, callObj.handler, callObj.errorHandler)())
            }
        })
        this._renewClient()
    }
    ...
    remoteMethod(methodName, options, handler, errorHandler){
        ...
        this.remoteCallQueue.push({emit:this._remoteHandShake(), consume:this._remoteHandShakeConsume(), error:errorHandler})
    ...
    _renewClient(){
        ...
        if (this.clientConnection === null){
            Proxy.logger.debug("_renewClient: Creating new client connection")
            this.clientConnection = createClient()
        }else{
            Proxy.logger.debug("_renewClient: Recreating client connection")
    ...
    dataCallback(){
        return (data)=>{
            ...
            msgs.forEach((msg, idx)=>{
                ...
                var errorCallback = remoteCallbacks.error
