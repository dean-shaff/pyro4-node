 ...
 */
const { initOptions } = require("./util.js")
...
class NameServer{
    ...
    list(options){
        options = initOptions(options, {
            prefix: null, regix: null,
            metadataAll: null, metadataAny: null,
            returnMetadata: null
        })
        if (
            options.prefix !== null ||
...
if (typeof require != 'undefined' && require.main == module) {
    var main = async (options)=>{
        ...
        ns = await startNs(options)
        console.log(`NameServer URI: ${ns.nameServer.lookup(NAMESERVER_NAME).str}`)
    }
    main()
}
